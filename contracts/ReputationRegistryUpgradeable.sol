...
contract ReputationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
  ...
  function giveFeedback(
    ...
    require(
      !IIdentityRegistry(_identityRegistry).isAuthorizedOrOwner(
        msg.sender,
        agentId
      ),
  ...
  function appendResponse(
    ...
    // Track new responder
    if (
      !$._responderExists[agentId][clientAddress][feedbackIndex][msg.sender]
    ) {
    ...
    )
  {
    ...
    for (uint256 i; i < clientList.length; i++) {
      ...
      for (uint64 j = 1; j <= lastIdx; j++) {
        ...
        if (emptyHash != tag1Hash && tag1Hash != keccak256(bytes(fb.tag1)))
          continue;
        if (emptyHash != tag2Hash && tag2Hash != keccak256(bytes(fb.tag2)))
          continue;
  ...
  function _countResponses(
    ...
    if (responders.length == 0) {
      ...
      address[] memory allResponders = $._responders[agentId][clientAddress][
        feedbackIndex
      ];
